...
MSG_TYPE(NewTestReq,1,{unsigned int a; unsigned int b;})
...
int main(int argc, char *argv[])
{
    ...
    qDebug() << QByteArray(request.getData(),request.getSize()).toHex();

    //ШАГ 1: Создание экземпляра запроса. (см. шаг 0)
    NewTestReq request1({0x1,0x1});
    qDebug() << QByteArray(request.getData(),request.getSize()).toHex();
    ...
    MsgHandler<TestReq,TestResp,ContextExample> handler(cmdProcFunc); //Обработчик запроса

    //ШАГ 4: Создание обработчика ответа
    MsgHandler<TestResp,StatResp,ContextExample> msgHandler(respProcFunc); //Обработчик ответа
